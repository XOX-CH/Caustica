    ...
    private void recordFrame(RtContext ctx, RtPipeline active, GpuTexture nativeColor) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush(); RtDebugLabels.Scope frameLabel = RtDebugLabels.scope(ctx, cmd, "composite frame")) {
            ...
            Float4 cloudLook3 = new Float4(
                    // Render-resolution multiplier scales the primary free-flight stride budget on the
                    // CPU: the shader only ever reads an absolute step count, so 1.0 reproduces the
                    // path-steps slider exactly and a higher value spends more (smaller) strides per
                    // shell crossing without touching the snapped stride computation.
                    (float) Math.clamp(Math.round(CausticaConfig.Rt.Cloud.PATH_STEPS.value()
                            * CausticaConfig.Rt.Cloud.RESOLUTION.value()), 1, 128),
            ...
            new WorldPushData(
                    ...
                    new Float4(
                        ...
                        (float) CausticaConfig.Rt.Water.WAVE_PRESET.value(),
                        CausticaConfig.Rt.Water.WATER_DISPERSION.value()),
                    ...
                    new Float4(
                        ...
                        CausticaConfig.Rt.Water.WAVE_BAND_2.value(),
                        CausticaConfig.Rt.Water.WAVE_BAND_3.value()),
                    new Float4(
                        CausticaConfig.Rt.Water.WAVE_BAND_4.value(),
                        CausticaConfig.Rt.Water.WAVE_BAND_5.value(),
                        CausticaConfig.Rt.Water.WAVE_BAND_6.value(),
                        CausticaConfig.Rt.Water.WAVE_BAND_7.value()),
                    new Float4(

// Temporal (midpoint) correction for DLSS-G multi-frame interpolation on Ada.
// SPDX-License-Identifier: MIT
// Copyright (c) 2026 ImDreamt (MFGAdaUnlock-RenoDx), ported into ngxshim.
//
// With multi-frame generation unlocked, 3x/4x produce the right NUMBER of frames
// but not the right CONTENT: the interpolation kernel shipped in nvngx_dlssg.dll
// blends the two source frames with a compiled-in 0.5 (104 `mul.ftz.f32 ...,
// 0f3F000000;` instructions in its PTX), so every generated frame lands at the
// temporal midpoint and 4x shows three identical half-way frames instead of
// frames at 1/4, 2/4, 3/4.
//
// The fix rewrites that kernel's PTX so the blend weight comes from the kernel's
// own temporal parameter instead of the constant:
//
...
//
// injected after the unique `$L__BB0_3:` label; the 104 midpoint constants then
// become %f136 (first half, current->previous) and %f134 (second half,
// previous->current).
//
// The fatbin also carries a precompiled sm_89 cubin, which the driver would load
// in preference to JIT-ing the edited PTX. The rebuilt fatbin is therefore
// TRUNCATED after the sm_89 PTX entry, dropping the cubin and forcing the JIT
// path; the PTX entry is re-emitted uncompressed (flags 0x41).
...
// Structural expectations. NVIDIA renamed every kernel in 310.9, but the temporal
// program itself is otherwise unchanged: the PTX grew by exactly the accumulated
// symbol-name delta. Keep separate, exact profiles so an unrelated provider
// cannot be admitted merely because it happens to contain a similar midpoint
// sequence.
...
inline bool BuildTemporalFatbin(const uint8_t* fat, size_t fat_size,
  ...
  if (raw != profile.ptx_bytes) {
    char buf[96];
    std::snprintf(buf, sizeof(buf), "PTX is %llu bytes, expected %zu",
                  static_cast<unsigned long long>(raw), profile.ptx_bytes);
    why = buf;
    return false;
  }
  ...
  if (marks.size() != kExpectedMidpoints) {
    char buf[96];
    std::snprintf(buf, sizeof(buf), "found %zu midpoint multiplies, expected %zu",
...
// Replaces the temporal kernel's fatbin in every descriptor that references it.
// The module carries eight identical descriptors in separate tables and we cannot
// tell which one the runtime will pick, so all of them are redirected. The rebuilt
// fatbin allocation intentionally lives for the process lifetime: the shim pins
// nvngx_dlssg.dll via LoadLibrary, so the redirected descriptors stay live until
// exit.
//
// Returns true and sets `slots` to the number of redirected descriptors; on
// failure the module is left untouched and `detail` carries the reason.
inline bool Apply(HMODULE module, size_t& slots, std::string& detail) {
  ...
  std::vector<uint64_t*> descriptor_slots;
  ...
  for (WORD i = 0; i < nt->FileHeader.NumberOfSections; ++i, ++section) {
    ...
    for (size_t off = 0; off + sizeof(uint64_t) <= size; off += sizeof(uint64_t)) {
      ...
      descriptor_slots.push_back(reinterpret_cast<uint64_t*>(sec + off));
    }
  }

  if (fat == nullptr || descriptor_slots.empty() || selected_profile == nullptr) {
  ...
  for (uint64_t* slot : descriptor_slots) {
    DWORD old_protect = 0;
  ...
  char buf[192];
  std::snprintf(buf, sizeof(buf), "redirected %zu %s descriptor(s) from a %zu-byte fatbin "
               "to a %zu-byte temporal-corrected rebuild",
               slots, selected_profile->descriptor_name, fat_size, rebuilt.size());
  detail = buf;
  return true;
}
